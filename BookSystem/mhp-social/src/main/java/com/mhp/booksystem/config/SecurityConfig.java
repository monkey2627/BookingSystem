...
/**
 * Spring Security 核心配置（mhp-social）。
 *
 * 整体策略：无状态 JWT 鉴权。
 ...
 *   /ws/**               WebSocket 握手端点，HTTP Upgrade 请求不携带 Header token；
 *                        STOMP 层的认证由 StompAuthChannelInterceptor 单独处理（CONNECT 帧）
 *   /api/post/feed       动态广场，支持可选认证：未登录可浏览全量 feed，
 *                        已登录时 Controller 内从 SecurityContext 读 userId 返回个性化内容
 *   /swagger-ui/**       Swagger 静态资源
 *   /swagger-ui.html     Swagger 入口页
 *   /v3/api-docs/**      OpenAPI JSON，Swagger UI 加载时请求
 * 其余所有路径（发帖、关注、评价、消息等）均需登录。
 */
@Configuration
        ...
        return http
                ...
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers(
                                ...
                                "/ws/**",             // WebSocket 握手：Upgrade 请求不带 Header token，
                                                      // STOMP 认证交给 StompAuthChannelInterceptor 处理
                                "/api/post/feed",     // 动态广场：支持未登录浏览（可选认证）
                ...
                //
                // 注意 /api/post/feed 的可选认证：
                //   JwtAuthenticationFilter 对任何路径都尝试解析 token（有 token 就写 context，没有就跳过）。
                //   /api/post/feed 在白名单里（permitAll），AuthorizationFilter 不会拒绝无 context 的请求。
                //   Controller 里检查 auth instanceof UsernamePasswordAuthenticationToken
                //   来判断是否已登录，有则返回个性化 feed，无则返回通用 feed。
