...
## 五、InnoDB 引擎
* 注意在innoDB引擎中，数据就是在聚簇索引的叶子节点而不是单独的一个磁盘文件存储，所以在有必要全表扫描的时候
* 实际上是全表扫描的聚簇索引
### 5.1 逻辑存储结构
...
```
# 核心问题：唯一索引等值查询，到底会不会“一个个找id”？
你的直觉：`where goods_id=1`，索引就要一条条遍历比对，怎么会没扫描行？
👉**你的误区：把「B+树索引点查询」和「索引范围遍历扫描」混为一谈。两者“扫描”完全不是一回事。**

B+树结构回顾：
- 叶子节点是有序索引条目，按主键/索引key排序；叶子之间有双向链表。
- **点查询（等值唯一索引）：走树的根→分支页→叶子页，直接定位key应该出现的位置，不是从第一行挨个遍历。**
- **遍历扫描：拿到叶子节点某起点之后，顺着叶子链表向后一条一条读取记录。**

> “扫描过的行都加锁”这句话里的**扫描 = 沿着叶子链表迭代读取一条条索引记录**。
> 单纯B+树树页查找、定位key的位置，**不等于扫描记录**。

## 1、两种查找方式对比
### ① 唯一索引等值查询：`goods_id=1`（goods_id唯一索引）
假设叶子页现有key：`5，10，20`，没有1。
InnoDB执行流程：
1. 从根页往下，通过页内二分查找，定位到叶子页面；
2. 在叶子页内部做页内二分，找到**key=1应该插入的间隙位置**（在5前面）；
3. 发现本页没有key=1这条索引记录；**直接结束查找，不会向后遍历下一条记录**。

✅做了：读取索引页，页内二分定位间隙。
❌没有做：顺着叶子链表，一条条读取物理索引记录。
👉**没有读取任何一条真实存在的索引记录，所以没有Record行锁。只给这个间隙加Gap锁。**

> 页的读取（IO） ≠ 扫描记录。
> 读索引页是找位置；扫描记录是迭代取出一条条索引行。

### ② 什么才叫真正的“扫描（遍历）”
两种场景会触发索引记录遍历扫描：
1. 范围查询：`where goods_id>1`；
2. 普通二级索引等值查询；
3. 全表扫描；
4. 优化器选择range访问方式。

示例：`where goods_id>1`，同样叶子是5,10,20
1. B+树定位到第一个大于1的key=5；
2. **然后顺着叶子节点链表，一条一条拿：5、10、20……逐条取出索引记录**，每取出一条真实索引记录，就加Next‑Key锁。
> 这个逐条拿记录的动作，才叫**扫描记录**。

这里哪怕where后server层过滤不匹配，InnoDB已经把这条索引记录读出来了，锁就加上，RR不会释放。
这就是课本那句话：**扫描过的行，就算where不匹配，锁不释放**。

> 关键点：这句话生效的前提：**已经把这条索引记录从叶子节点取出来交给上层了**。

---

## 2、再解释：为什么唯一索引等值找不到记录，不会向后扫描？
因为是**唯一索引**，语义：最多只能有一条匹配记录。
存储引擎的逻辑：
> 我来找key=1，只要在索引页定位该key的位置，找不到，就可以断定：整张表不存在这条记录。
> 唯一索引不需要继续往后读下一条记录，不可能还有第二个1。

👉 所以不会触发“逐条扫描叶子链表”。

对比：普通二级索引，非唯一。
`where age=10`，age不是唯一索引。
就算找到了一条age=10，还必须继续向后扫描叶子，看下一条是不是也是age=10，把全部匹配记录找出来。
这就是普通索引等值，就算命中记录，依然会带Gap锁的根源。

## 3、把两个经典规则重新翻译人话，消除冲突
### 规则A：RR隔离级别，扫描读取到的索引记录，即使MySQL Server层where过滤不满足，锁不会释放。
> 适用：range扫描、普通索引等值、全表扫描；**已经逐条取出索引记录**。
> 不适用：唯一索引等值查询，key不存在，没有取出任何索引记录。

### 规则B：唯一索引等值查询RR
1. 定位后找到记录：取出该记录（发生一次记录读取），Next‑Key临键锁退化为Record行锁。
2. 定位后找不到记录：只定位间隙，**没有取出任何索引记录**，不加Record锁，仅加Gap间隙锁。

> 规则B和规则A并不矛盾：规则A的场景根本没有发生。

## 4、举一个对比案例帮你彻底分清
表：goods_id唯一索引，存在5、10、20。没有1。

事务1 RR：
```sql
-- case1：唯一索引等值，不存在
update goods set ii=1 where goods_id=1;
```
动作：B+树页内二分定位间隙；**没有读取任何索引记录**；只有Gap锁。

```sql
-- case2：范围查询 >1
update goods set ii=1 where goods_id>1;
```
动作：定位起点key=5，**开始顺着链表扫描读取5、10、20这一条条索引记录**，每一条都加Next‑Key锁。就算上层where未来过滤，锁也保留。这里就触发规则A。

## 5、你最开始的理解错在哪
> ❌错误理解：where写了goods_id=1，引擎就会循环遍历每一行索引记录挨个比对id。
> ✅正确：B+树唯一索引点查询是二分定位，不是暴力遍历；找不到就直接结束，不会逐条遍历。
> “扫描行”特指**顺着叶子链表迭代取出一条条索引记录**，不是指读索引页、做页内二分查找。

## 补充面试常挖坑点
> Q：RR，唯一索引where id=1，id不存在，有没有扫描行？会不会触发“扫描过的行不释放锁”？
A：**没有扫描任何索引记录，不会触发那条规则。只有Gap间隙锁，无Record行锁。**

> Q：那什么场景才会出现：扫描到行，where不匹配，锁保留？
A：range范围、普通索引等值、全表扫描。例如普通索引`age=10 and xxx`，索引读到age=10，回表发现xxx不满足，RR下这条索引记录锁依旧持有。
