...
### Broker、Exchange、Queue 的层级关系

> 面试常见混淆点：Broker 和 Exchange 不是同一个东西。

```
Broker（整个 RabbitMQ 服务实例）
  └── vhost（命名空间，隔离不同业务）
        ├── Exchange（交换机：分拣转发，不存消息）
        ├── Queue（队列：真正存消息的地方）
        ├── Binding（Exchange ↔ Queue 的绑定规则）
        └── Channel（TCP 连接内的轻量逻辑通道）
```

- **Broker**：整栋大楼（RabbitMQ 进程），包含全部内部组件
- **Exchange**：大楼里的"分拣前台"，只做路由决策，不存消息
- **Queue**：大楼里的"储物房间"，真正持久化消息
- **Channel 信道**：一条 TCP 连接内划分多条逻辑通道。TCP 连接建立代价很高，不能每条消息新建 TCP；复用一条 TCP，用多个 Channel 做独立的 AMQP 操作，性能优。

**与 Kafka 的关键区别**：Kafka 没有交换机概念，生产者直接发给 topic。RabbitMQ（AMQP）必须经过 Exchange。
...
| **Queue** | 存储消息，消费者从这里取 |
| **Binding** | Exchange 和 Queue 的绑定关系，附带 BindingKey |
| **Consumer** | 从 Queue 获取消息，执行业务逻辑 |

**Routing-Key vs Binding-Key**：
- Routing-Key：生产者发消息时贴在消息上的标签
- Binding-Key：Exchange 绑定 Queue 时写的匹配规则
- Direct 模式：两者必须完全相同；Topic 模式：Binding-Key 可带通配符

### 完整消息流转

```
生产者 →(routing-key)→ Exchange
                          │
                  根据 binding 规则匹配
                          │
                       Queue（存储）
...
> **Exchange 不存储消息**。如果没有 Queue 与 Exchange 绑定，或路由不匹配，消息直接丢弃。
...
Producer → exchange(topic)
  bindingKey="order.*"  → Queue A    // 匹配 "order.create"，不匹配 "order.pay.alipay"
  bindingKey="order.#"  → Queue B    // 匹配以 "order." 开头的一切
  bindingKey="notify.#" → Queue C    // 匹配以 "notify." 开头的一切
```

**项目用法**：`schedule.exchange` 是 Topic 类型，`notify.#` 捕获所有 `notify.xxx` 路由键：
```
...
**规则**：根据消息的 headers（键值对）路由，忽略 RoutingKey。几乎不用。
...
**Publisher Confirms**：MQ 处理消息后，**异步**回调通知 Producer，不阻塞业务线程。

配置：
```yaml
spring:
  rabbitmq:
    publisher-confirm-type: correlated   # correlated：每条消息独立回调（推荐）
...
rabbitTemplate.setConfirmCallback((correlationData, ack, cause) -> {
    if (ack) {
        // 消息成功到达 Broker 并交给交换机
    } else {
        // Broker 明确拒绝了消息（交换机不存在、权限错误等）
        log.error("投递失败，原因：{}", cause);
        // 补偿：重发 or 记录数据库
    }
});
```

> `correlationData`：消息唯一 ID（发消息时传入），回调时用来定位是哪条消息。

#### ⚠️ 最高频考点：ack=true ≠ 消息进队列

**`ack=true` 只代表消息到达 Broker 并且交给了 Exchange，不代表消息进入 Queue！**

如果 Exchange 收到了消息，但是路由不到任何 Queue（无绑定），`ConfirmCallback` 依旧 `ack=true`，此时触发 `ReturnCallback`。

#### ⚠️ 最高频考点：ack=false ≠ 网络丢包

| 场景 | ConfirmCallback | ReturnCallback |
|------|----------------|----------------|
| **正常路由**（消息成功进队列）| `ack=true` | 不触发 |
| **路由失败**（Exchange 存在但无匹配 Queue）| `ack=true` | **触发**（消息被 MQ 丢弃）|
| **Exchange 不存在**（Broker 拒收）| `ack=false` | 不触发 |
| **网络丢包**（数据包没到 Broker）| **完全不执行** | **完全不执行** |

核心记忆：
- `ack=false`：前提是数据包**已到达 Broker**，只是 Broker 明确拒绝（交换机不存在、权限错误）
- 网络丢包：Broker 完全不知道这条消息存在，**没有任何回调**，既不 true 也不 false

#### ReturnCallback（路由失败退回回调）

```java
rabbitTemplate.setReturnsCallback(returned -> {
    log.error("消息路由失败，exchange={}, routingKey={}",
              returned.getExchange(), returned.getRoutingKey());
    // 触发代表消息在 MQ 侧已丢弃，需要业务层处理
});
```

`Return` 触发时，消息**已在 MQ 侧丢弃**，不会存入任何 Queue。

#### 网络丢包怎么兜底（Confirm 解决不了）

Confirm/Return 都依赖"数据包到达 Broker"这个前提，纯网络丢包时两个回调都不会执行。

**解决方案：本地数据库 + 定时补偿**：
1. 发消息前，DB 记录一条：`msgId、内容、状态=待确认`
2. 发消息，带上 `CorrelationData(msgId)`
3. Confirm 回调：`ack=true` → 更新 DB `投递成功`；`ack=false` → 更新 `投递失败`，重试
4. **定时任务**扫描 DB，找出超时仍为"待确认"的消息 → 判定为网络丢包 → 重新发送

> 只靠 Confirm + Return 不能 100% 保证生产者不丢消息，必须配合业务层数据库兜底。

**`correlated` vs `simple` 的区别**：`simple` 所有消息共享一个回调无法区分哪条失败；`correlated` 每条消息有独立 `CorrelationData`，可精确追踪。生产环境必须用 `correlated`。

### 环节二：MQ 本身（持久化）

MQ 收到消息后如果宕机，内存里的消息会丢失。三层持久化缺一不可：
...
> **⚠️ 重点**：消费者断开连接、消息没有 ACK，**不会**成为死信，消息重新回到队列等待重新投递。`requeue=true` 的消息**永远不会进入死信队列**，只是重新放回原队列。

### nack/reject 的 requeue 参数详解

```
requeue=true  → 消息退回原队列头部 → 马上再次投递给消费者 → 如果一直失败，死循环！
               永远不会进死信队列

requeue=false → 消息丢弃
               有死信队列（DLX）→ 转发到死信队列（消息不丢失，可人工排查）
               没有死信队列  → 彻底删除，永久丢失
```

> **线上经典 Bug**：`nack + requeue=true`，消息一直消费失败 → 疯狂重试 → CPU 打满。**务必配合死信队列使用 `requeue=false`**。

比喻：
- `requeue=true`：快递坏了，退回快递站，再一次次派送给你
- `requeue=false`：拒收快递；有垃圾桶（死信队列）就扔进去，没有直接扔掉
...
```java
// 死信交换机：Direct 类型
DirectExchange scheduleDeadExchange =
    ExchangeBuilder.directExchange("schedule.dead.exchange").durable(true).build();
...
缺点：所有消息必须用同一个 TTL（Queue 级别），不能每条消息设不同延迟。若每条消息设不同 TTL（Message 级别），由于 RabbitMQ 按队列头部过期，中间短 TTL 消息也无法提前过期。
...
**项目中的定时任务选型**：本项目用的是 XXL-Job（预约超时取消、每日提醒），而非延迟消息。XXL-Job 擅长周期性批量任务；延迟消息擅长"N 分钟后针对某条数据触发"的精细化延迟。
...
`msgId` 用 `IdUtil.fastSimpleUUID()` 生成，每次发消息时设置，MQ 重投时 msgId 不变。

**注意**：处理失败时删除幂等 key（`stringRedisTemplate.delete(idempotentKey)`），允许下次重投时再处理，避免因瞬时故障永久跳过这条消息。
...
2. Consumer 限流：`prefetch-count`（每次只预取 N 条）
...
spring:
  rabbitmq:
    listener:
      simple:
        prefetch: 1  # Consumer 每次只取 1 条，处理完再取下一条
```
...
Spring AMQP 默认使用 Java 原生序列化（`SimpleMessageConverter`）：
...
@Data
@Builder
@NoArgsConstructor   // ← 必须有，Jackson 反序列化需要无参构造
@AllArgsConstructor
...
| 参数 | 含义 | 效果 |
|------|------|------|
| `prefetch=N` | 每个 Consumer 线程最多预取 N 条未 ACK 的消息 | 控制单个消费者"手里拿着"几条消息 |
...
> GPU 串行场景需两者**同时设为 1**：只设 `concurrentConsumers=1` 但 `prefetch=5`，这一个线程会同时持有 5 条消息，仍可能资源竞争。
...
Consumer 方法抛出异常时，Spring AMQP 默认行为是**无限重试**，可能造成消息风暴。
...
public void consume(RenderMessage message) {
    taskService.markProcessing(message.getTaskId());
    try {
        doInference(message);
        taskService.markCompleted(message.getTaskId(), resultPath);
    } catch (Exception e) {
        log.error("任务 {} 推理失败", message.getTaskId(), e);
        taskService.markFailed(message.getTaskId(), e.getMessage());
        // ⚠️ 不重新抛出：失败已记录到 DB，消息正常 ACK
        // 若 throw → 消息无限重投 → GPU 持续被打 → 雪崩
    }
}
```

核心思想：**MQ 负责传递消息，DB 负责记录状态**。业务失败 ≠ 消息传递失败，两者不应混为一谈。
...
### ACK / NACK / Reject 完整对比

```java
channel.basicAck(deliveryTag, multiple);
// multiple=false：只确认当前这一条消息（多线程乱序消费必须用 false）
// multiple=true ：确认 deliveryTag 以及所有比它小的 tag 全部确认（批量，减少网络 IO）

channel.basicNack(deliveryTag, multiple, requeue);
// multiple：同上，支持批量拒绝
// requeue=true ：重新入队，不会进死信（⚠️ 小心死循环）
// requeue=false：丢弃，有死信队列则转发，否则永久删除

channel.basicReject(deliveryTag, requeue);
// 只能拒绝单条，没有 multiple 参数（这就是为什么有 basicNack）
```

#### ⚠️ multiple=true 的重大陷阱（面试高频）

`multiple=true` 会确认 tag ≤ 指定值的**全部**消息，**多线程乱序消费时严禁使用**：

```
收到 tag=5、tag=6
5 处理慢，6 先处理完，执行 basicAck(6, true)
→ multiple=true：tag≤6 全部确认，tag=5（还没处理完）也被删掉 → 消息丢失！
```

**顺序消费 + prefetch 多条** 时可以用 `multiple=true` 减少网络往返；**多线程异步消费**一定用 `multiple=false`，逐条 ACK。
...
TFAvatar 是一个基于 3D Gaussian Splatting 的人体虚拟形象重建系统。GPU 推理单次耗时 **5~30 秒**，若直接在 HTTP 请求线程内同步等待：
...
GPU 是独占资源，两个推理任务并发必然导致显存溢出。`prefetch=1` 确保 Consumer 手里始终只有 1 条消息，前一条处理完才取下一条。
...
**Q：Broker 和 Exchange 有什么区别？**

Broker 是整个 RabbitMQ 服务实例（进程），内部包含 vhost、Exchange、Queue、Binding、Channel 等所有组件。Exchange 只是 Broker 内部负责路由转发的组件，本身不存消息。消息 `ack=true` 代表消息到达 Broker 并且交付给了 Exchange，不等于消息进入 Queue。

**Q：ConfirmCallback 的 ack=true 是什么意思？是否代表消息进队列了？**

不是。`ack=true` 只代表消息成功到达 Broker 并交给了 Exchange，不代表消息进入 Queue。如果 Exchange 存在但路由不到任何 Queue，`ack` 依旧是 `true`，此时会触发 `ReturnCallback`。

**Q：ack=false 是网络丢包吗？**

不是。`ack=false` 的前提是消息数据包**已到达 Broker**，只是 Broker 明确拒绝（如交换机不存在、权限错误）。网络丢包时，Broker 完全感知不到这条消息，`ConfirmCallback` **完全不执行**，没有 true 也没有 false。

**Q：Confirm/Return 都不触发时怎么办（网络丢包场景）？**

需要生产者本地数据库记录消息状态（待确认），定时扫描超时仍为"待确认"的消息判定为网络丢包，重新发送。只靠 Confirm + Return 回调不能 100% 保证可靠性，必须配合业务层数据库兜底。

**Q：RabbitMQ 如何保证消息不丢失？**

三个环节三种保证：① 生产者 Publisher Confirm + Return Callback，加业务 DB 定时超时补偿 ② MQ 自身 Exchange/Queue/Message 持久化 ③ 消费者手动 ACK，处理完再确认。

**Q：nack requeue=true 和 requeue=false 有什么区别？requeue=true 会进死信队列吗？**

`requeue=true`：消息重新放回原队列头部，**不会进死信队列**，如果消息一直消费失败会造成死循环，CPU 打满，是线上经典 Bug。`requeue=false`：消息丢弃，有死信队列（DLX）则转发，否则永久删除。只有 `requeue=false`、TTL 过期、队列满溢出这三种情况才会触发死信路由。

**Q：basicAck 的 multiple 参数有什么用？什么时候不能用 true？**

`multiple=false`：只确认当前这一条消息。`multiple=true`：确认 deliveryTag 及所有比它小的 tag，批量确认，可减少网络 IO。**多线程乱序消费严禁 `multiple=true`**：若 tag=5 还未处理完，tag=6 先处理完调用 `basicAck(6, true)`，会把 tag=5 也一并确认删除，导致消息丢失。只有顺序消费场景才能用 `multiple=true`。
...
`simple`：所有消息共享一个确认回调，无法区分哪条消息失败。`correlated`：每条消息有独立的 `CorrelationData`，可以精确追踪哪条消息未确认。生产环境必须用 `correlated`。
